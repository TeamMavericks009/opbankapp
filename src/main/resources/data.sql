 ...
 * Description : Loaded two more rows into dbo.transaction, beautified the script..
 */
 ...
 *  2023-10-27    ELDHLP-67  Sarika Kondakindi  Load entities: , convert script to be rerunnable.
 *  2023-11-01    ELDHLP-67  Sarika Kondakindi  Loaded two more rows into dbo.transaction, beautified the script.
 * 
 */
do $$
declare
...
begin 
	...
	if not exists (select 1 
	                 from dbo.address a
	                limit 1) then 
       raise notice 'Data Inserting into dbo.address a';
	...
	if not exists (select 1 
       ...
       raise notice 'Data Inserting into dbo.users a';  	                 
	...
	if not exists (select 1 
	                ...
	                limit 1) then 
	   -------------*** dbo.bank_account ***-------------
       raise notice 'Data Inserting into dbo.bank_account a';    
   	...
   	if not exists (select 1 
	                 from dbo.linked_payee a
	                limit 1) then 
	   -------------*** dbo.linked_payee ***-------------     
       raise notice 'Data Inserting into dbo.linked_payee a';               
    ...
    if not exists (select 1 
	                 from dbo.cards a
	                limit 1) then
	                  raise notice 'Data Inserting into dbo.cards';
       -------------*** dbo.cards ***------------- 
       raise notice 'Data Inserting into dbo.cards a';  	                 
    ...
    if not exists (select 1 
	                 from dbo.user_login a
	                limit 1) then
       -------------*** dbo.user_login ***------------- 	                
       raise notice 'Data Inserting into dbo.user_login';
    ...
    if not exists (select 1 
	                 from dbo.user_login_history a
	                limit 1) then

       -------------*** dbo.user_login_history ***-------------    
       raise notice 'Data Inserting into dbo.user_login_history';      
    ...
    if not exists (select 1 
	                 from dbo.user_registration a
	                limit 1) then
       -------------*** dbo.user_registration ***------------- 	                
       raise notice 'Data Inserting into dbo.user_registration';
    ...
    if not exists (select 1 
	                ...
	                limit 1) then
       -------------*** dbo.bank ***------------- 	                
       raise notice 'Data Inserting into dbo.bank';
    ...
    if not exists (select 1 
	                 from dbo.user_feedback a
	                limit 1) then
       -------------*** dbo.user_feedback ***------------- 	                
       raise notice 'Data Inserting into dbo.user_feedback';
    ...
    if not exists (select 1 
	                ...
	                limit 1) then
       -------------*** dbo.sessions ***-------------	                
       raise notice 'Data Inserting into dbo.sessions';
    ...
    if not exists (select 1 
	                 from dbo.bot_response_history a
	                limit 1) then
       -------------*** dbo.bot_response_history ***-------------	                
       raise notice 'Data Inserting into dbo.bot_response_history';
    ...
    if not exists (select 1 
	                 from dbo.error_log a
	                limit 1) then
       -------------*** dbo.error_log ***-------------	                
	   raise notice 'Data Inserting into dbo.error_log';
    ...
    if not exists (select 1 
	                 from dbo.user_preferences a
	                limit 1) then
       -------------*** dbo.user_preferences ***-------------	                
	   raise notice 'Data Inserting into dbo.user_preferences';
    ...
    if not exists (select 1 
	                 from dbo.transaction_category a
	                limit 1) then
       -------------*** dbo.transaction_category ***-------------	                
	   raise notice 'Data Inserting into dbo.transaction_category';
    ...
    if not exists (select 1 
	                 from dbo.transaction_type a
	                limit 1) then
       -------------*** dbo.transaction_type ***-------------	                
	   raise notice 'Data Inserting into dbo.transaction_type';
    ...
    if not exists (select 1 
	                ...
	                limit 1) then
       -------------*** dbo."transaction" ***-------------	                
       raise notice 'Data Inserting into dbo."transaction"';

       insert into dbo."transaction"(transaction_id, transaction_no, bank_account_id, transaction_category_id, transaction_type_id, transaction_amount, participant_account_id, balance, status, user_id, transaction_date, description)
                              values(nextval('dbo.transaction_seq'),1,1,1,2,20.30, 1,200.30,'Success', 1, '2023-10-18 12:01:02','Money Transfer' ),
                                    (nextval('dbo.transaction_seq'),2,2,2,1,20.30, 2,200.30,'Success', 2, '2023-10-18 12:01:02','Money Transfer' ),
                                    (nextval('dbo.transaction_seq'),3,3,3,1,20.30, 3,200.30,'Success', 3, '2023-10-18 12:01:02','Money Transfer' ),
                                    (nextval('dbo.transaction_seq'),4,4,4,2,20.30, 4,200.30,'Success', 4, '2023-10-18 12:01:02','Money Transfer' ),
                                    (nextval('dbo.transaction_seq'),5,5,3,1,20.30, 5,200.30,'Success', 5, '2023-10-18 12:01:02','Money Transfer' );

       insert into dbo."transaction"(transaction_id, transaction_no, bank_account_id, transaction_category_id, transaction_type_id, transaction_amount, participant_account_id, balance, status, user_id, transaction_date, description)
                              values(nextval('dbo.transaction_seq'),2,1,3,1,50.30, 1,150.00,'Success', 1, '2023-10-19 15:07:08','Money Transfer' );

       insert into dbo."transaction"(transaction_id, transaction_no, bank_account_id, transaction_category_id, transaction_type_id, transaction_amount, participant_account_id, balance, status, user_id, transaction_date, description)
                              values(nextval('dbo.transaction_seq'),3,1,3,2,70.00, 3,220.00,'Success', 1, '2023-10-23 09:13:15','Money Transfer' );
